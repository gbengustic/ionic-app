...
save() {
  this.http.post('https://click-on-kaduna.herokuapp.com/api/student', this.userID)
  .subscribe((response)=>{
...
delete() {
  this.http.delete('https://click-on-kaduna.herokuapp.com/api/student/' + this.userID)
  .subscribe((response) => {
    const res: any = response;
    this.presentAlert(res.message);
  });
